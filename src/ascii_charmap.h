/* ascii_charmap.h - force cc65 to emit C literals as plain ASCII
 * bytes. The cbm targets remap literals to PETSCII, which breaks
 * byte-exact compares against on-disk data (pak magic, git object
 * headers, file names) and makes emitted text non-ASCII. Identity-
 * mapping every character keeps shared code honest on both hosts.
 * The C64 display path uses the lowercase charset where ASCII
 * bytes render correctly as-is. */
#ifdef __CC65__
